...
export default function Sidebar() {
  ...
  return (
    <div className={`fixed h-screen flex flex-col p-4 w-[20rem] shadow-lg border-r transition-all ${toggle ? "w-[5rem] items-center " : ""}`}>
        <div className='flex justify-between items-center'>
            <h1 className={`text-xl font-bold transition-all ${toggle ? "hidden" : ""}`}>Game<span className='text-main'>fy</span></h1>
          <div className='flex items-center'>
            <ModeToggle/>
            <ChevronFirst className={`text-black/50 hover:text-black cursor-pointer transition-all duration-500 dark:text-white/75 dark:hover:text-white  ${toggle ? "rotate-180" : ""}`} onClick={setToggle}/>
          </div>
        </div>
